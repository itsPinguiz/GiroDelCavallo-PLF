...
knightTour :: Int -> Board -> Position -> Int -> IORef (Int, Board) -> IO (Maybe Board)
knightTour n board (x, y) move ref = do
    let missing = n * n - move
    writeIORef ref (missing, board)
    if move == n * n 
        then return (Just board)
        else do
            let nextMoves = map tryMove knightMoves
            results <- mapM id nextMoves
            return $ listToMaybe $ catMaybes results
  where
    tryMove (dx, dy) = do
      let nextPos = (x + dx, y + dy)
      if isValid n board nextPos
        then knightTour n (updateBoard board nextPos move) nextPos (move + 1) ref
        else return Nothing
...
main = do
    ...
    result <- timeout (2 * 60 * 10^6) $ do
        ...
        then putStrLn "Posizione di partenza non valida." >> return Nothing
            ...
            knightTour size initialBoard start 1 ref
    case result of
